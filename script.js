document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DISCLAIMER (disabled; scripts preserved) ---
    // Kept as no-op functions for backward compatibility with older builds.
    function injectDisclaimerHTML() { return; }
    function initializeDisclaimer() { return; }

    // Defer helper preserved for compatibility
    function defer(fn) {
        if (typeof fn !== 'function') return;
        const run = () => setTimeout(() => { try { fn(); } catch (_) {} }, 250);
